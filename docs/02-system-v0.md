...
5. **Two axes, and the reading runs out.** Vertical is the time of the story: **one paragraph per screen**, from beginning to end; when a story ends, the next one follows if you keep scrolling, and it announces itself in the top band, beside the spine. On a story's last paragraph, and only if another story remains, a floating cue at the bottom —"Siguiente historia" over a slowly bobbing arrow— says there is more before you get there; tapping it does what the scroll does. *(Added 23-sep-2026: the announcement only appears once you have already arrived, and a scroll nobody suspects does not exist.)* When there are no stories left the scroll reaches the bottom and stays there. Horizontal is the other versions of that same moment. *(Revised 03-sep-2026: the previous version paginated, and on a phone nobody found the tap. Revised again 04-sep-2026: the scroll now chains story after story, which §7 used to forbid. What separates this from a feed is not the length of the corpus but that **each story ends, says so, and the next is announced** — the boundary is a threshold, not a hairline. A cap on paragraphs per story is what guarantees it: three. Amended 06-sep-2026: the chain no longer repeats. Each story enters once and the reading finishes; before this it drew another story at random for as long as you scrolled, which is a feed with better manners. The order is shuffled — never by age or by backing — so freshly seeded and orphaned stories do not sink to the end.)*
