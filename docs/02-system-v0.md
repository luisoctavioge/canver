...
**Date:** 03-sep-2026
**Decisions:** topology = *legible tree*. Unit = text, 140 characters at most (text-only MVP). Reading experience = **two axes, not a feed**: vertical is the time of the story (one paragraph per screen), horizontal the alternate versions of that moment.
...
|---|---|
| **Fragment** | The only publishable unit, also called a line. 140 characters at most (100 until 25-sep-2026). Every fragment has exactly one parent. |
| **Paragraph** | Exactly five lines by five different hands. Not published by anyone: it closes on the count. |
| **Story** | Three paragraphs at most: fifteen lines by fifteen different people, and then it ends. *(08-sep-2026; five until then.)* |
...
|---|---|---|
| Turns per period | 2 a day (stated, never counted) | Enough to have judgement, too few to make noise. *(3 a week until 05-sep-2026, 1 a day until 06-sep-2026.)* They are not handed out by a nightly job: each person collects what they are owed the next time they show up, so there is no scheduler to maintain. The bottom bar says the cadence — «2 turnos por día» — and not how many you have left: counting turns is still counting. It only changes its mind when you have none, because the invitation to write disappears with them and silence there reads as a fault. *(06-sep-2026.)* |
| Accumulation cap | 5 | You can save, not hoard. |
| Cost of continuing | 1 turn | The default action is the cheapest one. |
| Cost of seeding | 1 turn, on the blank page at the end of the reading | **Starting costs what continuing costs; what protects continuing is where the blank page sits** — after the last story, once every one has been offered to you. *(25-sep-2026. It was 3 turns, unlocked after N continuations: with two turns a day that was a whole day's savings for one line, and the test starts with only three stories.)* |
...
0. **One welcome, once.** The first time a person opens their link, a title and two sentences over the paper say what to do — «Bienvenido a Canver. Continúa una historia o empieza una para que otros la sigan. Tienes 2 oportunidades de escribir por día.» — and a tap anywhere dismisses them. It never opens on its own again, on any device — only when you tap the circled question mark in the top band: the database remembers it per person (`welcome()`, `people.welcomed_at`), since the prototype keeps nothing in the browser. It is the only instruction text in the product. *(Added 26-sep-2026 at the owner's request, ahead of sending the links.)*
...
5. **Two axes, and the reading runs out.** *(Horizontal axis switched off 25-sep-2026 — `HORIZONTAL = false` in the prototype — to test linear stories first: no version dots, no sideways gesture, canon at every moment. The code stays; the flag brings it back.)* Vertical is the time of the story: **one paragraph per screen**, from beginning to end; when a story ends, the next one follows if you keep scrolling, and it announces itself in the top band, beside the spine. On a story's last paragraph, and only if another story remains, a floating cue at the bottom —"Siguiente historia" over a slowly bobbing arrow— says there is more before you get there; tapping it does what the scroll does. *(Added 23-sep-2026: the announcement only appears once you have already arrived, and a scroll nobody suspects does not exist.)* When there are no stories left, one more screen: a blank page — «Empieza una historia…» — where your line becomes the first of a new story, for one turn; the cue on the last story's last paragraph says «Empieza una historia» instead of «Siguiente historia». With no turns left there is no blank page, and the scroll reaches the bottom and stays there. *(Blank page added 25-sep-2026.)* Horizontal is the other versions of that same moment. *(Revised 03-sep-2026: the previous version paginated, and on a phone nobody found the tap. Revised again 04-sep-2026: the scroll now chains story after story, which §7 used to forbid. What separates this from a feed is not the length of the corpus but that **each story ends, says so, and the next is announced** — the boundary is a threshold, not a hairline. A cap on paragraphs per story is what guarantees it: three. Amended 06-sep-2026: the chain no longer repeats. Each story enters once and the reading finishes; before this it drew another story at random for as long as you scrolled, which is a feed with better manners. The order is shuffled — never by age or by backing — so freshly seeded and orphaned stories do not sink to the end.)*
6. **The top band belongs to the story, the text belongs to the moment.** A story is three paragraphs; a moment holds three versions at most. What ends is the story, not the reading. Everything that is true of the whole story lives in one fixed band above the text: *(The bookmark is switched off for now — `GUARDAR = false`, 26-sep-2026 — and a circled question mark sits in its place, reopening the welcome. Saved stories stay in the database; the flag brings the bookmark back.)* the announcement that a story is opening, the spine, and the bookmark that saves the **story**, not the paragraph. The spine is a stack of short rules, one per paragraph the story actually has — the current one wider and in the accent colour. One fixed indicator, never repeated per screen. *(Was five dots in the bottom bar until 05-sep-2026; they read as an ellipsis and sat too far from the text. Revised 06-sep-2026: the spine used to draw all five paragraphs, the unwritten ones as dashed outlines, and the announcement sat inside the text. A story seeded that morning has one paragraph, and four dashed slots said it was four fifths missing when what it was is new; and with the announcement gone from the column, a story's first paragraph now starts at exactly the height an echo would.)*
7. **What arrived while you were away reads warm.** A line written since your last visit carries the same soft tint as a line you have just written yourself, and it goes out after a moment on that screen: reading it is exactly what stops it being new. No badge, no count of unread things, no list — the news is the text. Your own lines are never new to you, and the last visit is remembered by the database, not the browser, so it follows you between phone and laptop. It only advances when half an hour has passed, so reloading in the same sitting does not erase what you have not reached yet. *(Added 06-sep-2026. It is what makes coming back worth it, and it is the first thing in the product that rewards being a reader rather than a writer.)*

8. **You write inside the text, not on a form.** The last version is always blank: swiping past the ones that exist lands on an empty one, and that is the invitation to write it — reaching the end of the versions and being asked to add one are the same gesture. Any version with fewer than five lines can also take your line. In both cases the caret sits exactly where the line will go, in the same type, under the last line you were reading. No second screen, no field, no button: the limit simply stops accepting, and Enter publishes. While you write —and only then— a faint «48 / 140» sits under the paragraph, and turns red when the line is full; it disappears the moment you stop. Reading still counts nothing. *(Counter added 25-sep-2026 at the owner's request; until then there was none.)* *(Added 05-sep-2026, after a first attempt sent writing to its own screen with labels and dividers. Writing is reading with a caret in it.)*
   **Your own line can be corrected until someone continues it.** Tap it and you write over it, in place; Enter saves, Escape or tapping away leaves it as it was, and it costs no turn. Only you see which of your lines are still loose — a faint dotted underline. Once somebody writes after it, the next hand has answered those exact words, and changing them would change what that person said: it belongs to the chain. The database enforces this (`edit_line`), not the page. *(Added 24-sep-2026.)*
8. **Versions are ordered left to right and the canon is the leftmost one.** Swiping right to left moves forward through them and left to right comes back. It does not wrap: the two ends are hard stops. *(It was one direction with a wrap until 05-sep-2026, so that the axis could be learnt without explaining it. The dots now show where you stand and where the ends are, which is the job the wrap was doing badly: you could not tell whether you were going or returning.)*
...
10. **A fork does not cut a paragraph short.** It used to, so that the fork would coincide with a paragraph break; but a paragraph is now always five lines, and that takes precedence. *(Revised 04-sep-2026. How a fork opens is still to be designed — see §8.)* Above each paragraph, a row of small dots says which version of this moment you are on, and the last one is dotted: that is the one nobody has written yet. Three versions per moment and no more, so when the three exist the dotted one is gone. *(Was a single `*` in the margin until 05-sep-2026; it said a fork existed but not how wide it was nor where in it you stood.)*
...
7. ~~**How many paragraphs at most can a story have?**~~ Resolved 08-sep-2026: **three**, and **three versions per moment**. What makes a story finite is the cap, and a finite story is half of what keeps the reading from being a feed; the other half is that the chain of stories also ends. Five paragraphs asked twenty-five different hands to finish one story, which with twenty-seven people and two turns a day is a story that stays open forever. Five versions did the opposite damage: they spread the hands across moments that never advance, and the branches were already dying three or four fragments in.

---

## 9. How it gets validated

A one-week test with 20–30 people: one tree, 3 seeds, 140 characters, 3 turns per person, a viewing window of 2.
