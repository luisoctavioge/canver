...
6. **Reading has two axes, and it does not stop.** Vertical is the time of the story: one paragraph per screen, from beginning to end, and when a story ends the next one follows if you keep scrolling. Horizontal is the other versions of that same moment. No cards, no avatars, no timestamps, no counters. *(Revised 04-sep-2026: the scroll is now endless, which this rule used to forbid. What keeps it from being a feed is that each story is finite, ends, and says so, and that the one that follows is announced — never served silently. A cap on paragraphs per story is what makes that true; it is still to be defined.)* The seam between fragments is a space, never a visible line.
