...
1. **You can only publish by answering.** There is no standalone post. Seeding a new story is possible, but only once you have read to the end of the ones there are.
2. **Answers are scarce.** Two turns a day, five saved at most. Scarcity is not a limitation: it is what turns every answer into an editorial decision. *(Three a week until 05-sep-2026, then one a day, then two from 06-sep-2026. Each move bought less scarcity and more participation, on purpose: the first test asks **how people write**, and with a single daily turn someone who arrives once and finds a paragraph where their hand already is goes home with nothing. Worth tightening again after the week — the ratchet has only turned one way so far, which is the direction to watch.)*
3. **Seeding sits at the end of the reading.** It costs one turn, the same as continuing, and there is no right to unlock: what keeps people continuing is that the blank page only appears after the last story, once every one of them has been offered to you. *(Amended 25-sep-2026 by the owner. It used to cost 3 turns against 1, unlocked by having continued; with two turns a day that was a whole day's savings for one line, and the test starts with three stories — if nobody can open another, the corpus stays at three. Worth watching: if people seed instead of continuing, this is the rule to tighten.)*
4. **The like backs a paragraph, and the canon follows it.** More likes, more canon; ties go to the older one. **Withdrawn from the product on 06-sep-2026 for the first test** — the rule and the `likes` table stand, and canon still reads from them, but there is no way to give one. The test asks how people write, and a heart invites you to have an opinion about what is there before it invites you to continue it. With no likes at all the canon falls back to the older version, which is what the tie-break already said. *(Amended 05-sep-2026. The rule used to forbid likes and votes outright, and canon was written by subtree weight — turns spent, not thumbs. The reason for the change is worth keeping: the like was not designed, it was felt while reading versions of the same moment on a phone and wanting to back one.)* **Fecundity** — how many continuations were born from your fragment — is still the metric that describes a hand, and it is still unfalsifiable, because every point of it costs somebody a scarce turn. No count of likes is ever shown: what you see is not how many backed a version, it is which version they made canon.
...
6. **Reading has two axes, and it runs out.** Vertical is the time of the story: one paragraph per screen, from beginning to end, and when a story ends the next one follows if you keep scrolling — until there are no stories left, and then one more screen: a blank page to start a new story, if you still have a turn. Without one, the scroll simply reaches the bottom. Horizontal is the other versions of that same moment. **The horizontal axis is switched off for now** (25-sep-2026, `HORIZONTAL = false` in the prototype): the test first asks whether a linear story holds up, so there are no version dots, no sideways gesture, and every moment follows canon. Nothing was removed — flipping the flag brings it all back. Saving a story is off the same way (`GUARDAR = false`, 26-sep-2026); a circled question mark takes the bookmark's place and reopens the first-visit welcome. No cards, no avatars, no timestamps, no counters. *(Revised 04-sep-2026 to chain story after story, which this rule used to forbid; 05-sep-2026 dropped the closing screen, so the boundary is carried by the announcement of the next story. 23-sep-2026 added a floating "Siguiente historia" cue with a moving arrow at the bottom of a story's last paragraph, only while another story remains: a scroll nobody suspects does not exist. Amended 06-sep-2026: **each story appears once and the reading ends.** It used to loop, picking another story at random for as long as you scrolled, and a reel that starts over is a feed under another name. What makes this an anthology is that it can be finished. The order is shuffled, never by age or backing — that is the anti-Matthew rule, so freshly seeded and orphaned stories do not pile up at the end.)* The seam between fragments is a space, never a visible line.
...
| Unit | **Text, 140 characters at most** | A fragment, also called a line. *(100 until 25-sep-2026: enough for one sentence, an image and a turn, never two ideas.)* Short ones weave paragraphs; long ones read as separate voices and fall back into the shape of a list. |
...
db/02-seed.sql          five stories, one line each (three since db/14)
...
db/12-ciento-cuarenta.sql  a line goes up to 140 characters
db/13-sembrar-cuesta-uno.sql  starting a story costs one turn
db/14-tres-historias.sql  ONE-TIME: drops two seeds, leaving three; never replay
db/15-bienvenida.sql    the first-visit welcome, remembered per person
...
**The corpus starts at one line.** *(06-sep-2026.)* Five stories, one sentence each, nothing continued. Reading something already finished gives you nothing to continue, and continuing is the only thing the test measures — so what the test starts from is five seeds and five turns' worth of appetite. The five sentences were already written, by their own hands, in the sample fiction; choosing which of them open rather than continue was the only editing done, and rule 8 stands. *(25-sep-2026: three stories, not five — the tren, the biblioteca and noviembre. The rest get seeded by the people in the test, on the blank page at the end of the reading.)*

**Everything through `db/15` is applied.** The scripts are no longer pasted into the SQL editor by hand: they run against the project through Supabase's management API, which connects as `postgres` and therefore goes straight over the row policies. So the rule that protects the corpus can no longer be the schema, it has to be the habit — **migrations get applied; one-time destructive scripts stay by hand.** `db/06-una-sola-linea.sql` is the one that must never be replayed: it deletes every fragment hanging off a seed, which is why it was deliberately not written as a migration. `db/14-tres-historias.sql` is the same kind: it ran once, at the owner's explicit request, and deletes two stories.
